...
class PresentasePendidikanLuarSekolah extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Luar Sekolah';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Luar Sekolah')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Luar Sekolah')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
