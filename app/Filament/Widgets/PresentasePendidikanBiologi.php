...
class PresentasePendidikanBiologi extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Biologi';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Biologi')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Biologi')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
