...
class PresentaseBiologi extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Biologi';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Biologi')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Biologi')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
    ...
    public static function canView(): bool
    {
        ...
        if ($fakultas !== 'Matematika dan Ilmu Pengetahuan Alam') {
            return false;
        }
