...
class PresentaseTeknikInformatika extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Teknik Informatika';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Teknik Informatika')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Teknik Informatika')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
