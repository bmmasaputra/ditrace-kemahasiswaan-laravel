...
class PresentaseIHukum extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Ilmu Hukum';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Ilmu Hukum')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Ilmu Hukum')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
