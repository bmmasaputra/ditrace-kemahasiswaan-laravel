...
class PresentaseIlmuAdministrasiNegara extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Ilmu Administrasi Negara';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Ilmu Administrasi Negara')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Ilmu Administrasi Negara')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
    ...
    public static function canView(): bool
    {
        ...
        if ($fakultas !== 'Ilmu Sosial dan Ilmu Politik') {
            return false;
        }
