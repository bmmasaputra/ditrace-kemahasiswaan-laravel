...
class PresentaseKedokteran extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Kedokteran';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Kedokteran')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Kedokteran')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
