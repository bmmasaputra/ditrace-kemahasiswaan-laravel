...
class PresentaseTeknologIndustriPert extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Teknologi Industri Pertanian';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Teknologi Industri Pertanian')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Teknologi Industri Pertanian')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
