...
class PresentasePendidikanFisika extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Fisika';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Fisika')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Fisika')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
