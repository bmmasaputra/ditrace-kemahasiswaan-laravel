...
class PresentaseAkutansi extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Akuntansi';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Akuntansi')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Akuntansi')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
