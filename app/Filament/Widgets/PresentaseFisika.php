...
class PresentaseFisika extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Fisika';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Fisika')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Fisika')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
