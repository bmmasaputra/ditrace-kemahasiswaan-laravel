...
class PresentaseAgribisnis extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Agribisnis';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Agribisnis')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Agribisnis')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
