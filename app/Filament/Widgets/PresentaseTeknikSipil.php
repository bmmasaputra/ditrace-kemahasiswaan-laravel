...
class PresentaseTeknikSipil extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Teknik Sipil';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Teknik Sipil')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Teknik Sipil')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
