...
class PresentasePendidikanKimia extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Kimia';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Kimia')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Kimia')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
