...
class PresentasePendidikanBahasaInggris extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Bahasa Inggris';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Bahasa Inggris')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Bahasa Inggris')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
