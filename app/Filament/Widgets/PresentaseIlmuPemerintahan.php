...
class PresentaseIlmuPemerintahan extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Ilmu Pemerintahan';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Ilmu Pemerintahan')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Ilmu Pemerintahan')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
