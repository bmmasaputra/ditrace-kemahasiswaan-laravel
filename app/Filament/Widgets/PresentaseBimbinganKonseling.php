...
class PresentaseBimbinganKonseling extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Bimbingan dan Konseling';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Bimbingan dan Konseling')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Bimbingan dan Konseling')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
