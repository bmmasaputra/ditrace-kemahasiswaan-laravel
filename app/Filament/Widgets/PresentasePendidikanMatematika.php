...
class PresentasePendidikanMatematika extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Pendidikan Matematika';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Pendidikan Matematika')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Pendidikan Matematika')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
