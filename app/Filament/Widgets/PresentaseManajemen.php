...
class PresentaseManajemen extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Manajemen';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Manajemen')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Manajemen')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
