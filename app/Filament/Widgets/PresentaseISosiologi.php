...
class PresentaseISosiologi extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Sosiologi';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Sosiologi')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Sosiologi')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
