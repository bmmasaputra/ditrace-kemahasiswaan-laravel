...
class PresentaseKimia extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Kimia';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Kimia')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Kimia')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
