...
class PresentaseKehutanan extends ChartWidget
{
    use InteractsWithPageFilters;

    protected ?string $heading = 'Presentase Kehutanan';
    ...
    protected function getData(): array
    {
        ...
        $counttotalMahasiswa = User::where('jurusan', 'Kehutanan')
            ->where('thn_lulus', $tahunLulus)
            ->count();
        $countTracerStudy = TracerStudy::where('prodi', 'Kehutanan')
            ->where('thn_lulus', $tahunLulus)
            ->where('f8', '!=', '')
            ->count();
